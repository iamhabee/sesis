...
fetchLoanGroupDetails=(id)=>{
  ...
  fetch(getConfig('getLoanActivities')+ id +`?token=`+user.token, requestOptions)
  .then(async response => {
    const data = await response.json();
    if (!response.ok) {
        const error = (data && data.message) || response.statusText;
...
fetchLoanDetails=(id)=>{
    const {user} = this.state
    let requestOptions = this.getRequestOpt
    fetch(getConfig('loanRepaymentsDetails')+ id +`?token=`+user.token, requestOptions)
      .then(async response => {
      const data = await response.json();
      if (!response.ok) {
          const error = (data && data.message) || response.statusText;
...
handleCreateManage = (id) => {
  ...
  fetch(getConfig('getLoanGroupDetails')+ id +`?token=`+ this.state.user.token, this.getRequestOpt)
      .then(async response => {
      const data = await response.json();
      if (!response.ok) {
          const error = (data && data.message) || response.statusText;
...
handleCreateRepayment = (id) => {
  ...
  fetch(getConfig("loanBalance")+id+"?token="+user.token, this.getRequestOpt)
      .then(async response => {
      const data = await response.json();
      if (!response.ok) {
          const error = (data && data.message) || response.statusText;
          ...
          <Grid item lg={8} md={8} sm={12} xs={12}>
            <div className="pb-5 px-2 bg-default" style={{border:1, borderStyle:"solid", borderColor:"#04956a", borderBottomRightRadius:20,
              ...
              loan_details.map((data, index) => (
                <LoanCards key={index} data={data} repayment={()=>this.handleCreateRepayment(data.loan_id)} view={()=>this.handleCreateManageLoan(data.loan_id)}/>
              )):
              isFetching? <Typography variant="h6">Loading...</Typography>:
              loan_details.length == 0?
                <Typography variant="h6">You currently do not have any ongoing loan</Typography>:
      ...
      <Card className="px-6 pt-2 pb-4">
        <Grid container spacing={2}>
          <Grid item lg={12} md={12} sm={12} xs={12}>
            ...
            <Typography>Loading...</Typography>:
            <div className="pb-5 pt-5 px-2 bg-default" style={{border:1, borderStyle:"solid",     borderColor:"#04956a", borderBottomRightRadius:20, borderTopLeftRadius:20}}>
              <LoanDetailsCard data={loan_approval} approvals={repayment_details} />
            </div>
            }
          </Grid>
        </Grid>
